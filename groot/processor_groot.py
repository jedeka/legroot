...
def _build_eagle_processor(tokenizer_assets_repo: str = DEFAULT_TOKENIZER_ASSETS_REPO) -> ProcessorMixin:
    ...
    # import os
    # pkl_dump(proc, f'{os.getcwd()}/0_proc.pkl')
    
    
    # Fix incorrect Mistral regex pattern that causes out-of-bounds token indices
    if hasattr(proc, 'tokenizer') and hasattr(proc.tokenizer, '_tokenizer'):
        try:
            proc.tokenizer = proc.tokenizer.__class__.from_pretrained(
                str(cache_dir), 
            ...
            pass
    # print('*'*30); print(cache_dir); print(dir(proc)); print('*'*30)
...
def collate(features: list[dict[str, Any]], eagle_processor: ProcessorMixin) -> dict[str, Any]:
    ...
    for key in keys:
        ...
        if key == "eagle_content":
            ...
            # NOTE: custom
            if "pixel_values" in eagle_inputs and isinstance(eagle_inputs["pixel_values"], list):
                all_tiles = [tile for sample_tiles in eagle_inputs["pixel_values"] for tile in sample_tiles]
                eagle_inputs["pixel_values"] = torch.stack(all_tiles)
