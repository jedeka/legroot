...
class Eagle25VLProcessor(ProcessorMixin):
    ...
    def replace_media_placeholder(
        ...
        def replace_in_text(text):
            ...
            def repl(match):
                ...
                if media_type == "image":
                    ...
                    # pkl_dump(image_inputs, f'{os.getcwd()}/a.pkl')
                    # if DEBUG: print(image_inputs); print('*'*30)
                    # pv = torch.stack(image_inputs["pixel_values"][0])
                    # num_all_tiles = pv.shape[0]

                    # More robust fix:
                    pv = image_inputs["pixel_values"]
                    if isinstance(pv, list):
                        # pv is [[tile1, tile2, ...], [tile1, ...], ...]
                        # flatten all tiles across batch
                        all_tiles = [tile for sample_tiles in pv for tile in sample_tiles]
                        pv = torch.stack(all_tiles)
                    num_all_tiles = pv.shape[0]
                    image_inputs["pixel_values"] = pv
                        # print(len(pv)); print('*'*30)
                        
                        # if isinstance(pv, list):
                        #     import torch
                        #     pv = torch.stack(pv) if isinstance(pv[0], torch.Tensor) else torch.tensor(pv)
                        #     image_inputs["pixel_values"] = pv

                    # print(num_all_tiles); print('*'*30)
        ...
        if len(unified_frame_list) > 0:
            ...
            # image_sizes = torch.cat([frame["image_sizes"][0] for frame in unified_frame_list])
            # print(unified_frame_list)
            # if DEBUG: pkl_dump(unified_frame_list, 'b.pkl')
            # pixel_values = torch.cat([torch.stack(frame["pixel_values"][0]) for frame in unified_frame_list])
            image_sizes = torch.cat([torch.tensor(frame["image_sizes"]) for frame in unified_frame_list])
        else:
            pixel_values = None
            image_sizes = None
        return (
